...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image_feature_crop, label=None):
        ...
        fix_logists = logit_scale * image_feature_crop @ fix_label_features.t()
...
for epoch in range(start_epoch, epochs):
    for idx, batch in enumerate(tqdm(train_dataloader)):
        ...
        loss = model(image_feature_crop,pred_label)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    print('epoch %d : train_loss = %f' %(epoch, loss))
    torch.save(model.prompt_learner.state_dict(), '/home/ths/zero-shot-clip/neural-motifs/prompt-ckpt/vg_to/LAMM/prompt_learner_params_%d.pth' % epoch)
    ...
    with open('/home/ths/zero-shot-clip/neural-motifs/result/vg_to/LAMM/label_prompt_epoch'+str(epoch)+'.pickle', 'wb') as file:
